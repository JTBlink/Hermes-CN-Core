...
| **P-051** | `.github/workflows/release-runtime.yml`、`hermes_cli/port_lock.py`、runtime release 与端口锁测试 | **让 Linux 冻结 runtime 兼容 CN Desktop 的安装包基线。** Linux runtime 固定在 Ubuntu 22.04 与 Python 3.14.6 构建；managed runtime 子进程通过 `HERMES_PORT_LOCK_OWNER_PID` 接受存活 Desktop 父进程预占的 9120/8644/8645 端口锁。 | Ubuntu 24.04 构建的 `libpython` 依赖 GLIBC 2.38，在 Ubuntu 22.04（GLIBC 2.35）首次启动即崩溃；同时 Desktop 启动前正确预占 managed 端口集合，但 Core 原先把父进程的锁误判为外部冲突并拒绝启动。 | CN Desktop 发布与集成行为，不按原样上游。 |
