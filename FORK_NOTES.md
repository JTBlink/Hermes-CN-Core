...
| **P-051** | `.github/workflows/release-runtime.yml`, `hermes_cli/port_lock.py`, runtime-release and port-lock tests | **Make the frozen Linux runtime compatible with the CN Desktop installer baseline.** Linux runtime builds are pinned to Ubuntu 22.04 and Python 3.14.6, and a managed runtime child accepts the 9120/8644/8645 lock set pre-claimed by its live Desktop parent through `HERMES_PORT_LOCK_OWNER_PID`. | Ubuntu 24.04 builds embedded a `libpython` requiring GLIBC 2.38 and crashed on Ubuntu 22.04 (GLIBC 2.35). Separately, Desktop correctly reserves its managed port set before spawn, but Core treated the parent's reservation as a foreign live process and refused to start. | CN Desktop release/integration behavior; not intended for upstream as-is. |
